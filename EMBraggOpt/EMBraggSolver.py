...
class WPEMsolver(object):
    def __init__(
        ...
        os.makedirs('WPEMFittingResults', exist_ok=True)
    ...
    def cal_output_result(self,):
        ...
        for task in range(len(self.initial_peak_file)):
            read_ini_peak(self.initial_peak_file[task], task)
        ...
        if len(o_x) == len(f_x) and len(f_x) == len(b_x):
            ...
            for task in range(len(self.Lattice_constants)):
                hkl_data_task = pd.read_csv('./WPEMFittingResults/hkl{task}_{year}.{month}.{day}_{hour}.{minute}.csv'.format(task=task,
            ...
            # save result files 
            with open(os.path.join('WPEMFittingResults','WPEMfittingProfile_{year}.{month}.{day}_{hour}.{minute}.csv'.format(year=namey,
                                       month=nameM,day=named, hour=nameh,minute=namem)),'w') as wfid:
                for j in range(len(theta_data)):
                    print(theta_data[j], end=', ', file=wfid)
                    print(float(y[j]), file=wfid)

            with open(os.path.join('DecomposedComponents', 'fitting_profile.csv'),'w') as wfid:
                for j in range(len(theta_data)):
                    print(theta_data[j], end=', ', file=wfid)
                    print(float(y[j]), file=wfid)

            with open(os.path.join('WPEMFittingResults','WPEMPeakParas_{year}.{month}.{day}_{hour}.{minute}.csv'.format(year=namey, month=nameM,
            ...
            with open(os.path.join('DecomposedComponents', 'sub_peaks.csv'),'w') as wfid:
                print('wi', end=',', file=wfid)
                print('Ai', end=',', file=wfid)
                print('mu_i', end=',', file=wfid)
                print('L_gamma_i', end=',', file=wfid)
            ...
            with open(os.path.join('WPEMFittingResults', 'LatticeConstances_{year}.{month}.{day}_{hour}.{minute}.csv'.format(year=namey,
                                         month=nameM,day=named,hour=nameh,minute=namem)), 'w') as wfid:
    ...
    def up_parameter(self, hkl_data,two_theta, intensity,area, min_i, i_obser, bac,lmd,):
        ...
        if self.MODEL == 'REFINEMENT':
            ...
            if type(self.Var) == float :
                ...
                with open(os.path.join('DecomposedComponents','upbackground.csv'),'w') as wfid:
                    for j in range(len(two_theta)):
                        print(two_theta[j], end=', ', file=wfid)
                        print(float(bac_up[j]), file=wfid)
            else:
                ...
                with open(os.path.join('WPEMFittingResults', 'upbackground_{year}.{month}.{day}_{hour}.{minute}.csv'.format(year=namey,
                        month=nameM,day=named, hour=nameh, minute=namem)), 'w') as wfid:
                    for j in range(len(two_theta)):
                        print(two_theta[j], end=', ', file=wfid)
                        print(float(bac_up[j]), file=wfid)
        ...
        # Model = 'ANALYSIS', the background strength is frozen
            ...
            with open(os.path.join('WPEMFittingResults','Other_Phase_peaks{year}.{month}.{day}_{hour}.{minute}.csv'.format(year=namey,
                                          month=nameM, day=named, hour=nameh, minute=namem)),'w') as wfid:
                for j in range(len(two_theta)):
                    print(two_theta[j], end=', ', file=wfid)
            ...
            plt.ylabel('I (a.u.)')
            plt.legend()
            plt.savefig('./WPEMFittingResults/Residual_Intensity_{year}.{month}.{day}_{hour}.{minute}.png'.format(year=namey,
        ...
        plt.ylabel('I (a.u.)')
        plt.legend()
        plt.savefig('./WPEMFittingResults/WPEM_fittingresult_{year}.{month}.{day}_{hour}.{minute}.png'.format(year=namey,
                    month=nameM,day=named, hour=nameh, minute=namem),dpi=800)
        plt.show()
        plt.clf()
        ...
        plt.legend()
        plt.savefig('./WPEMFittingResults/WPEM_log_likelihood_{year}.{month}.{day}_{hour}.{minute}.png'.format(year=namey,
                    month=nameM,day=named, hour=nameh,minute=namem),dpi=800)
        plt.show()
        plt.clf()
        ...
        plt.legend()
        plt.savefig( './WPEMFittingResults/WPEM_Rfactor_{year}.{month}.{day}_{hour}.{minute}.png'.format(year=namey, month=nameM,
...
def read_ini_peak(data_file, Num):
