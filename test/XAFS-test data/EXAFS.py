...
class EXAFS(object):
    """
    This class is the core solver of EXAFS module in WPEM 

    Contribution and suggestions are always welcome. You can also contact the authors for research collaboration.
    """
    def __init__(self,XAFSdata,  power = 2, distance = 5, k_point = 8,window_size=2,hop_size=0.5,de_bac = False):
        """
    ...
    def fit(self, Ezero = None, first_cutoff_energy=None,second_cutoff_energy=None):
        ...
        add_index = find_first_value_greater_than(_energy, _energy[0]+50)
        ...
        sample_rate = (K_space[1]-K_space[0])/10
        _, _, Sxx = spectrogram(K_space, fs=sample_rate, window='hann', nperseg=self.window_size, noverlap=self.hop_size)
        plt.imshow(10 * np.log10(Sxx), aspect='auto', cmap='viridis', origin='lower', extent=[0, len(K_space)/sample_rate, 0, sample_rate/2])
        plt.colorbar(label='Power/Frequency (dB/Hz)')
        plt.ylabel('radial distance (A\u00B0)')
        plt.xlabel(f' |\u03c7(R)| (A\u00B0-{self.power+1})', )
        plt.show()

        with open(os.path.join('XAFS/EXAFS/',
                                'FineStructure.csv'), 'w') as wfid:
